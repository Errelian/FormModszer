...
THEORY ListPreconditionX IS
  ...
  List_Precondition(Machine(CompanyHelper),find_largest_income_day)==(state = READ_DATA & size(income_array) = 5 & dom(income_array): FIN(INT) & !ii.(ii: dom(income_array) => ii: INT));
  List_Precondition(Machine(CompanyHelper),find_days_below_minimum)==(state = READ_DATA & size(income_array) = 5 & card(dom(income_array)) = size(income_array))
END
&
THEORY ListSubstitutionX IS
  Expanded_List_Substitution(Machine(CompanyHelper),find_days_below_minimum)==(state = READ_DATA & size(income_array) = 5 & card(dom(income_array)) = size(income_array) | @(day_indexes_below_minimum$1).(day_indexes_below_minimum$1: FIN(1..5) & card(day_indexes_below_minimum$1)<=size(income_array) & !ii.(ii: day_indexes_below_minimum$1 => ii: dom(income_array) & income_array(ii)<minimum_income) & !ii.(ii: dom(income_array) & income_array(ii)<minimum_income => ii: day_indexes_below_minimum$1) ==> day_indexes_below_minimum:=day_indexes_below_minimum$1));
  Expanded_List_Substitution(Machine(CompanyHelper),find_largest_income_day)==(state = READ_DATA & size(income_array) = 5 & dom(income_array): FIN(INT) & !ii.(ii: dom(income_array) => ii: INT) | @(highest_income_index$0).(highest_income_index$0: income_array~[{max(ran(income_array))}] ==> highest_income_index:=highest_income_index$0));
  ...
  List_Substitution(Machine(CompanyHelper),find_largest_income_day)==(highest_income_index:: income_array~[{max(ran(income_array))}]);
  List_Substitution(Machine(CompanyHelper),find_days_below_minimum)==(day_indexes_below_minimum: (day_indexes_below_minimum: FIN(1..5) & card(day_indexes_below_minimum)<=size(income_array) & !ii.(ii: day_indexes_below_minimum => ii: dom(income_array) & income_array(ii)<minimum_income) & !ii.(ii: dom(income_array) & income_array(ii)<minimum_income => ii: day_indexes_below_minimum)))
END
&
...
&
THEORY ListSetsX IS
  Set_Definition(Machine(CompanyHelper),STATUS_TYPE)==({INIT,READ_MIN,READ_DATA});
...
THEORY ListOfIdsX IS
  List_Of_Ids(Machine(CompanyHelper)) == (STATUS_TYPE,INIT,READ_MIN,READ_DATA | ? | highest_income_index,day_indexes_below_minimum,state,minimum_income,expense_array,income_array | ? | read_min_income,populate_financial_data,find_largest_income_day,find_days_below_minimum | ? | ? | ? | CompanyHelper);
...
&
THEORY SetsEnvX IS
  Sets(Machine(CompanyHelper)) == (Type(STATUS_TYPE) == Cst(SetOf(etype(STATUS_TYPE,0,2))))
END
&
THEORY ConstantsEnvX IS
  Constants(Machine(CompanyHelper)) == (Type(INIT) == Cst(etype(STATUS_TYPE,0,2));Type(READ_MIN) == Cst(etype(STATUS_TYPE,0,2));Type(READ_DATA) == Cst(etype(STATUS_TYPE,0,2)))
END
&
